...
Cocaine_Out <-
  ...
  # Initial object
  synthetic_control(outcome = lRatCoc, # The first, simple outcome
...
Cocaine_Out %>%
  plot_trends() +
  labs(title="Synthetic and Observed Rate of Cocaine Admissions",
       y="Log Cocaine Admission Rate",
       x="Year")
  ggsave(here("Figures/Figure_Trend_Cocaine.png"))
  
Cocaine_Out %>%
  plot_differences() +
  labs(title="Difference between Synthetic and Observed",
       y="Difference in Log Cocaine Admission Rate",
       x="Year")
ggsave(here("Figures/Figure_Difference_Cocaine.png"))

Cocaine_Out %>%
  plot_mspe_ratio() +
  labs(title="Ratio of Pre/Post Treatment Mean Squared Prediction Error",
       x="State FIPS, ordered")
ggsave(here("Figures/Figure_MSPERat_Cocaine.png"))
